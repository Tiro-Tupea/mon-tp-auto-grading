...
mot = donnees["ex2_mot"]
resultat_ex2 = " ".join(f"{c}:{i}" for i, c in enumerate(mot))
...
resultat_ex4 = "majuscule" if 65 <= ord(lettre) <= 90 else "minuscule"
...
resultat_ex6 = sum(1 for c in phrase if 48 <= ord(c) <= 57)
...
# "1352" → 1 + 3 + 5 + 2 = 11
resultat_ex7 = sum(ord(c) - 48 for c in chiffres_text)
...
for c in nombre_text:
    resultat_ex8 = resultat_ex8 * 10 + (ord(c) - 48)
resultat_ex8 = 0
...
# "123456" → True
resultat_ex9 = all(48 <= ord(c) <= 57 for c in texte)
...
def texte_vers_nombre(t):
    n = 0
    for c in t:
        n = n * 10 + (ord(c) - 48)
    return n
resultat_ex10 = texte_vers_nombre(num1_text) + texte_vers_nombre(num2_text)
...
n = nombre
while n > 0:
    chiffres.append(str(n % 10))
    n //= 10
resultat_ex11 = chiffres[::-1]
...
for op in ['+', '-', '*', '/']:
    if op in expression:
        gauche, droite = expression.split(op)
        a = texte_vers_nombre(gauche)
        b = texte_vers_nombre(droite)
        if op == '+': resultat_ex13 = a + b
        elif op == '-': resultat_ex13 = a - b
        elif op == '*': resultat_ex13 = a * b
        elif op == '/': resultat_ex13 = a / b
        break
resultat_ex13 = resultat_ex13