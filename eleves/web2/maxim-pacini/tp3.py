...
resultat_ex3 = 0
nbvoyelle = 0
while nbvoyelle < len(mot):
    if mot[nbvoyelle] in voyelles:
...
resultat_ex11 = ""
dld = 0
while dld < len(chaine):
    if len(resultat_ex11) == 0 or chaine[dld] != resultat_ex11[-1]:
        resultat_ex11 = resultat_ex11 + chaine[dld]
    dld = dld + 1
...
while i < len(phrase):
    if phrase[i] != ' ':
        motcourant = motcourant + phrase[i]
    else:
        if len(motcourant) > len(motlong):
            motlong = motcourant
        motcourant = ""
    i = i + 1
if len(motcourant) > len(motlong):
...
while i < len(phrase):
    if phrase[i] != ' ':
        nombrechar = nombrechar + 1
    i = i + 1

resultat_ex15 = {
    "nombre_mots": nombremots,
    "mot_le_plus_long": motlong,        
    "nombre_caracteres": nombrechar,   
}